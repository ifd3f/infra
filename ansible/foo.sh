...
if [ -f /etc/debian_version ]; then
	...
	if [ -f /etc/lsb-release -a -n "`cat /etc/lsb-release 2>/dev/null | grep -F -i LinuxMint`" ]; then
		...
		echo "deb http://download.zerotier.com/debian/bionic bionic main" >/tmp/zt-sources-list
	...
	echo "deb http://download.zerotier.com/debian/bionic bionic main" > sudo tee /etc/apt/sources.list.d/zerotier.list
