source "qemu" "vyos" {
  ...
  boot_command      = [
    ...
    "curl -o /opt/vyatta/etc/config/config.boot -sSL http://{{ .HTTPIP }}:{{ .HTTPPort }}/config-initial.boot<enter><wait>",
    ...
    "<enter><wait10>",  # Which one should I copy to sda? [/opt/vyatta/etc/config.boot.default]:
    ...
    # Enable SSH to signal to Packer that we're done
    "configure<enter>",
