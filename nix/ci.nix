...
let
  ...
  nixosNodesForSystem =
    ...
    mapAttrs'
      (
        ...
        let
          cfg = nixosSystem.config.astral.ci;
        in
        {
          ...
          value = {
            ...
            build = "nixosConfigurations.${hostname}.config.system.build.toplevel";
            run = mapNullable (
              _: "nixosConfigurations.${hostname}.config.astral.ci.run-package"
            ) cfg.run-package;
            deploy = mapNullable (
              _: "nixosConfigurations.${hostname}.config.astral.ci.deploy-package"
            ) cfg.deploy-package;
          };
        }
      )
      (
        filterAttrs (
          hostname: nixosSystem: nixosSystem.pkgs.system == system && nixosSystem.config.astral.ci.enable
  ...
  homeManagerNodeForSystem = system: {
    ...
    build = mapAttrsToList (key: _: "homeConfigurations.${key}.activationPackage") (
      filterAttrs (_: home: home.pkgs.system == system) self.homeConfigurations
    );
  };
  ...
  devShellNodeForSystem = system: {
    ...
    build = mapAttrsToList (key: _: "devShells.${system}.${key}") self.devShells.${system};
  };
...
rec {
  ...
  ssh-deploy-targets = sort lessThan (
    filter (x: x != null) (
      mapAttrsToList (_: system: system.config.astral.ci.deploy-to) self.nixosConfigurations
    )
  );
  ...
  nodes =
    {
      ...
      surface-kernel = {
        ...
        system = "x86_64-linux";
        prune-runner = true;
        build = "nixosConfigurations.shai-hulud.config.boot.kernelPackages.kernel";
      };
    }
