{ lib }:
let
  nixFlags = "--accept-flake-config --show-trace --log-lines 10000";
in
with lib;
rec {
  ghexpr = v: "\${{ ${v} }}";
  ...
  makeJob =
    ...
    let
      info = systems.elaborate system;
    in
    if info.isx86_64 then
      makex86Job envargs key spec
    # TODO: support aarch64 jobs
    else
  ...
  makex86Job =
    ...
    if build == [ ] && run == null && deploy == null then
    ...
    else
      {
        ...
        runs-on = if system == "x86_64-darwin" then "macos-latest" else "ubuntu-latest";

        needs =
          let
            missingDeps = filter (d: !(hasAttr d nodes)) needs;
          in
          if missingDeps != [ ] then
        ...
        steps =
          let
            ...
            buildStep = {
              ...
              run =
                let
                  ...
                  installables = map (attr: ''"$TARGET_FLAKE#"'' + escapeShellArg attr) buildList;
                  args = concatStringsSep " " installables;
                in
                "GC_DONT_GC=1 nix build ${nixFlags} --fallback ${args}";
              env.TARGET_FLAKE = ghexpr "env.TARGET_FLAKE";
            };

            runStep = {
              name = "Run ${run}";
              run = ''GC_DONT_GC=1 nix run ${nixFlags} "$TARGET_FLAKE#$run_flake_attr"'';
              env = {
                run_flake_attr = run;
            ...
            deployStep = {
              ...
              run = ''GC_DONT_GC=1 nix run ${nixFlags} "$TARGET_FLAKE#$deploy_flake_attr"'';
              "if" = ghexpr "github.event_name == 'push' && github.ref == 'refs/heads/main'";
              env = {
