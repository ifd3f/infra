...
let
  ...
  forEachL = lib.forEach [
    "h"
    "Left"
  ];
  forEachD = lib.forEach [
    "j"
    "Down"
  ];
  forEachU = lib.forEach [
    "k"
    "Up"
  ];
  forEachR = lib.forEach [
    "l"
    "Right"
  ];
  ...
  forEachDirKey =
    f: (forEachL (f "left")) ++ (forEachD (f "down")) ++ (forEachU (f "up")) ++ (forEachR (f "right"));
    ...
    config = {
      keybindings = (
        lib.mkMerge (
          ...
          in
          workspaceBinds ++ focusDirBinds ++ moveDirBinds ++ miscBinds
        )
      );
